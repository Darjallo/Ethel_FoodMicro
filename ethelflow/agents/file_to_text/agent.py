    ...
    def handle(self, request_json: Dict[str, Any]) -> Dict[str, Any]:
        ...
        try:
            gf = self.fs.get_last_version(
                metadata={"tenant": tenant, "collection": collection, "path": file_path}
            )
            file_bytes = gf.read()
            ...
            traceback.print_exc()
            result.update(
                {"status": 500, "error": f"Error retrieving file from GridFS: {exc}"}
            )
            return result
        ...
        try:
            ...
            traceback.print_exc()
            result.update({"status": 500, "error": f"Failed to write temp file: {exc}"})
            return result
        ...
        try:
            ...
            if extension == ".pdf":
                ...
                total_text = "".join(
                    d.page_content or "" for d in extracted_docs
                ).strip()
            ...
            try:
                ...
                pass
            result.update(
                {"status": 500, "error": f"Error extracting text from file: {exc}"}
            )
            return result
        ...
        # 7) Return success
        result.update({"status": 200, "text": full_text})
        return result
