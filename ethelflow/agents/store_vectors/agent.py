    ...
    def handle(self, request_json: Dict[str, Any]) -> Dict[str, Any]:
        ...
        if tenant and file_id and file_id.startswith(tenant + "/"):
            ...
            try:
                ...
                rsp.update(
                    {"status": 400, "error": "file_id must be 'tenant/collection/path'"}
                )
                return rsp
        else:
            rsp.update({"status": 400, "error": "Missing 'file_id' (and/or 'tenant')."})
            return rsp
        ...
        if not (isinstance(texts, list) and all(isinstance(t, str) for t in texts)):
            rsp.update({"status": 400, "error": "'texts' must be list[str]."})
            return rsp
        if not (isinstance(embeds, list) and all(isinstance(v, list) for v in embeds)):
            rsp.update(
                {"status": 400, "error": "'embeddings' must be list[list[float]]."}
            )
            return rsp
        if len(texts) != len(embeds):
            rsp.update({"status": 400, "error": "texts/embeddings length mismatch"})
            return rsp

        # ---------- 3) Open / create Chroma collection ----------
        chroma = open_chroma_for(
            tenant=tenant, collection_name=collection_name, emb_method="ada3large"
        )
        if chroma is None:
            rsp.update(
                {
        ...
        # ---------- 6) Insert ----------
        try:
            coll.add(ids=ids, embeddings=embeds, metadatas=metadatas, documents=texts)
        except Exception as e:
            traceback.print_exc()
            rsp.update({"status": 500, "error": f"Insertion failed: {e}"})
            return rsp
