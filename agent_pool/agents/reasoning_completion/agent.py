...
class ReasoningCompletionAgent:
    ...
    def __init__(self):
        ...
        if not endpoint:
            raise RuntimeError("Environment variable AZURE_ENDPOINT is missing or empty")
        ...
        if not self.azure_deployment:
            raise RuntimeError("Environment variable AZURE_CHAT_DEPLOYMENT is missing or empty")
    ...
    def handle(self, request_json: Dict[str, Any]) -> Dict[str, Any]:
        ...
        if not isinstance(raw_msgs, list) or not all(isinstance(m, dict) for m in raw_msgs):
        ...
        if reasoning_effort is not None:
            if not isinstance(reasoning_effort, str) or reasoning_effort.lower() not in (
    ...
    def stream(self, request_json: Dict[str, Any]) -> Generator[bytes, None, None]:
        ...
        if reasoning_effort is not None:
            if not isinstance(reasoning_effort, str) or reasoning_effort.lower() not in (
                ...
                yield size + chunk + b"\r\n"
                return
            reasoning_prefs = {"reasoning_preferences": {"reasoning_effort": reasoning_effort.lower()}}
